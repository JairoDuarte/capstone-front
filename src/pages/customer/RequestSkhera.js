...
const getCoordinates = async address => {
  try {
    const responce = await geocodeByAddress(address);
    const coordinates = await getLatLng(responce[0]);
    return coordinates;
  } catch (error) {
  ...
  return items.map(item => {
    return (
      <>
        <Form.Field>
          <Button
            style={{ fontWeight: 'normal', textAlign: 'left', marginRight: '0.0em', width: '100%', marginLeft: '0em' }}
            color="blue"
            primary={false}
            animated
          >
            <Button.Content visible>
              {' '}
              <Icon name="asterisk" /> <span style={{ padding: '0px 30px', fontSize: '18px' }}>{item}</span>
            </Button.Content>
...
export default class RequestSkhera extends Component {
  ...
  handleSelectAddress = async () => {
    if (this.state.addressFrom !== '' && this.state.addressTo !== '') {
      const fromCoordinates = await getCoordinates(this.state.addressFrom);
      const toCoordinates = await getCoordinates(this.state.addressTo);
      this.setState({
  ...
  setDistance = async distance => {
    ...
    try {
      const response = this.state.price
        ? await api.post(`/api/skhera/price`, { distance, to: price, deliver: this.state.deliver })
        : {};
      ...
      <>
        ...
        >
          ...
          <Grid.Column width={8} style={{}}>
            ...
            >
              <Form.Field>
                <label style={{ color: '#909090', fontFamily: 'Ropa Sans', fontSize: '16px', fontWeight: 'normal' }}>
              ...
              <Form.Field>
                ...
                <Input
                  action={{ onClick: () => this.addItem(), icon: 'add', basic: true }}
                  iconPosition="left"
                  icon="archive"
                  labelPosition="right"
                  onChange={this.handleInputChange}
              ...
              <Form.Group>
                <Form.Field style={{}}>
                  ...
                  <Select
                    ...
                    style={{ minWidth: '10em', fontSize: '16px', fontWeight: 'normal' }}
                    type="select"
                    placeholder="ASAP"
                  />
                </Form.Field>
              ...
              <Form.Button
                ...
                type="submit"
                color="grs"
                style={{ marginTop: '1em', marginLeft: '0em', width: '100%', height: '' }}
                disabled={
                  !this.state.price ||
          ...
          <Grid.Column width={8}>
            <Form style={{ marginBottom: '1em' }}>
              <PlacesAutocomplete
                style={{ color: '#909090', fontFamily: 'Ropa Sans', fontSize: '16px', fontWeight: 'normal' }}
                value={this.state.addressFrom}
                onChange={addressFrom => this.setState({ addressFrom })}
                ...
                {({ getInputProps, suggestions, getSuggestionItemProps, loading }) => (
                  <div>
                    <Form.Field>
                      ...
                      <Input
                        {...getInputProps({
                          placeholder: '3416 Tenmile Road, Waltham, Massachusetts, 3 floor ...',
              ...
              <PlacesAutocomplete
                ...
                onChange={addressTo => this.setState({ addressTo })}
                onSelect={this.handleSelectAddress}
              >
                {({ getInputProps, suggestions, getSuggestionItemProps, loading }) => (
                  <div>
